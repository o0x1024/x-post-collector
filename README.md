...
4. 打开一个或多个X标签页并登录。
5. 点击扩展图标，打开侧边栏。
6. 点击“选择目录”完成保存目录授权；未选择保存目录时，“开始采集”会被拒绝。
...
- 未选择保存目录时，点击“开始采集”会被直接拒绝，运行日志给出提示，“保存目录”区域会高亮提醒；
- 开始前会再次校验目录写入授权；授权失效（例如浏览器重启、权限被重置）时同样拒绝开始，并提示重新授权；
- 该限制确保每次采集会话都有可写入的目标目录，避免帖子只堆积在后台“待写入”队列而无法落盘。
